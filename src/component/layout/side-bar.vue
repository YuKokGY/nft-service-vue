<template>
  <div class="app-sidebar">
    <div v-if="!elMenuCollapse" class="logo">CMS</div>
    <div v-else class="mobile-logo">CMS</div>
    <div style="margin-bottom:50px">
      <div v-if="showSidebarSearch" style="margin-top: 15px">
        <div v-if="!showSearchList" class="search-display" @click="toSearch"><i class="el-icon-search"></i></div>
        <el-select
          v-if="showSearchList"
          ref="searchInput"
          v-model="sidebar"
      ...
      >
        <template v-for="item in sideBarList">
            ...
            <template v-for="subItem in item.children">
              ...
              >
                ...
                <router-link
                  ...
                  class="circle third"
                >
                  <el-menu-item :index="idMap[grandchildItem.name]" class="subMenuContent" style="padding-left: 80px;">
              ...
              <router-link v-else :key="subItem.name" :to="subItem.path" class="circle">
                <el-menu-item :index="idMap[subItem.name]" class="subMenuContent" style="padding-left: 60px;">
                  {{ subItem.title }}
                </el-menu-item>
              </router-link>
            </template>
          ...
          <router-link v-else :key="item.name" :to="item.path">
            <el-menu-item :key="idMap[item.name]" :index="idMap[item.name]" class="subMenuContent">
              <i v-if="!filterIcon(item.icon)" :class="item.icon"></i> <img v-else :src="item.icon" class="imgIcon"/>
              <span slot="title">{{ item.title }}</span>
            </el-menu-item>
          </router-link>
