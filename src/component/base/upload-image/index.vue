  ...
  <div v-loading="loading" class="upload-imgs-container">
    <template v-for="(item, i) in itemList">
      <template v-if="item.display">
        <div :key="item.id" v-loading="item.loading" :style="boxStyle" class="thumb-item">
          <el-image :fit="fit" :src="item.display" class="thumb-item-img" style="width: 100%; height: 100%;"></el-image>
          <div class="info">
            <i
              v-if="item.file"
          ...
          <div class="control">
            <i v-if="!disabled" class="el-icon-close del" title="删除" @click.prevent.stop="delItem(item.id)"></i>
      ...
      <template v-else>
        ...
        >
          ...
          <div style="margin-top: 1em;" v-html="rulesTip.join('<br>')"></div>
        </div>
      </template>
    </template>
    <input
...
<script>
import {checkIsAnimated, createId, getFileType, isEmptyObj} from './utils'
